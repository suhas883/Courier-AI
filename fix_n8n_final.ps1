...
try {
    ...
    Write-Host "Connected! Found $($workflows.Count) workflows."
}
catch {
...
function Upsert-Workflow ($name, $file) {
    ...
    if ($existing) {
        ...
        try {
            $response = Invoke-RestMethod "$n8nUrl/workflows/$($existing.id)" -Method PUT -Headers $headers -Body $jsonContent
        ...
        try {
            $new = Invoke-RestMethod "$n8nUrl/workflows" -Method POST -Headers $headers -Body $jsonContent
...
Upsert-Workflow "LiveTrackings MASTER (V24 MULTILINGUAL)" "n8n_v24_MULTILINGUAL.json"
